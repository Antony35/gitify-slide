<!--
╔══════════════════════════════════════════════╗
║       SLIDE 3 — La solution : Gitify         ║
╚══════════════════════════════════════════════╝
-->

<div class="flex flex-col items-center justify-center h-full">

<img
src="/public/onboarding.png"
alt="Gitify onboarding — Quel est votre niveau ?"
class="rounded-xl object-contain"
style="max-height: 72%; max-width: 90%; box-shadow: 0 24px 40px rgba(0,0,0,0.6); border: 1px solid rgba(255,255,255,0.06);"
v-motion="{ initial: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 800, delay: 100 } } }"
/>

  <p class="mono text-lg text-mint"
    v-motion="{ initial: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 800, delay: 300 } } }">
    La première question que GitHub ne pose jamais.
  </p>

</div>

<!--
NOTES ORATEUR — Slide 3
La réponse c'est Gitify.
Avant même de te montrer un seul projet, on te pose cette question.
Quel est ton niveau ? Quels langages tu maîtrises ? Tu veux des issues ouvertes ou découvrir de nouveaux repos ?
À partir de là, on peut construire un feed personnalisé.
C'est ça la différence : on part de toi, pas du repo.
-->
